function [SPns,ptns,pns] = depth_gammaG(SP0,pt0,p0,SP,pt,p,bassin)
...
[gammal,gammau] = gammaG_vals(SP0,pt0,p0,SP(c),pt(c),p(c),bassin);
e = gammau - gammal;
...
elseif (e<0&&c<n)
    ...
    iter = 0;
    success = 0;
    [gammal_d,gammau_d] = gammaG_vals(SP0,pt0,p0,SP(c_d),pt(c_d),p(c_d),bassin);
    e_d=gammau_d - gammal_d;
    ...
    while success == 0
        ...
        [gammal,gammau] = gammaG_vals(SP0,pt0,p0,SPc0,ptc0,pc0,bassin);
        ec0 = gammau - gammal;
        p1 = 0.5*(p(c) + pc0);
        ez1 = (e- ec0)/(pc0 - p(c));
        p2 = 0.5*(pc0 + p(c_d));
        ...
        if abs(ec0) <= 5e-5 && eps <= 5e-3
            ...
            niter = iter;
        elseif iter > 10
            [SPns,ptns,pns,niter] = e_solve_gammaG(SP,pt,p,[e e_d],[c c_d],SP0,pt0,p0,bassin);
            disp(['part: ', int2str(1)]);
            success = 1;
        else
            pc_0 = pc0;
...
elseif (e>0&&c>1)
    ...
    success = 0;
    iter = 0;
    [gammal_u,gammau_u] = gammaG_vals(SP0,pt0,p0,SP(c_s),pt(c_s),p(c_s),bassin);
    e_s=gammau_u - gammal_u;
    ...
    while success == 0
        ...
        if abs(ec0) <= 5e-5 && eps <= 5e-3
            ...
            niter = iter;
        elseif iter > 10
            [SPns,ptns,pns,niter] = e_solve_gammaG(SP,pt,p,[e_s e],[c_s c],SP0,pt0,p0,bassin);
            disp(['part: ', int2str(2)]);
            success = 1;
        else
            pc_0 = pc0;
