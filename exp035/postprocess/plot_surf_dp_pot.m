...
for icolorscale=1:1
    ...
    for ifig=1:nfig
        ...
        while (isp<=nsp) && ( ip <=nit) && (iit<=size(vv,1))
            ...
            title(['pressure difference p_{\sigma_{',num2str(round(pns3d(ip,ilat,ilon)),'%d'),'}}-p_{\omega} [dbar] (surface nr. ',num2str(ip),')'])
        ...
        print('-dpng','-r200',['../figures/dp_rp_3d/',num2str(ifig,'%02i')])
    end
end
