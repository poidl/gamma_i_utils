...
for icolorscale=1:1
    ...
    for ifig=1:nfig
        ...
        while (isp<=nsp) && ( ip <=nit) && (iit<=size(vv,1))
            ...
            title(['pressure difference p_{\omega,s^2}-p_{\omega,d\rho} [dbar] (surface nr. ',num2str(ip),')'])
            xlabel(['depth at backbone: ',num2str(pns3d(ip,ilat,ilon)),' m'],'fontsize',18)
            text(-20,95,txt(isp),'fontsize',18')

            iit=iit+1; isp=isp+1;
        end

        print('-dpng','-r200',['../figures/dp_omega_omega/',num2str(ifig,'%02i')])
    end
end
