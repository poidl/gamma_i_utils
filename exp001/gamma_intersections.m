...
for ii=1:nxy
    ...
    if any(~isnan(s(:,ii))) && any(~isnan(s_(:,ii)))
        ...
        [dum1,dum2,p_n]=depth_ntp_iter(s(:,ii)',ct(:,ii)',p(:,ii)', s_e, ct_e, p_e);
        ...
        r(ibot)=nan; % actually 0, but the lower cast bottle is undefined; don't include in system of equations.
        k_e(ibot)=nan; % actually nz
